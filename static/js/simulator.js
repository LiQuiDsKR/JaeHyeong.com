document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleCardClick(event) {
        ...
        } else { // 카드를 클릭했을떄, 하지만 강화대기 상태가 아닌 경우
            // 그 카드에 대한 정보를 보여주는 모달을 띄워줌
            $('#cardModal').modal('show');
            const modalContent = document.getElementById('card-modal-content');
            modalContent.innerHTML = createCardModalContent(card);
            document.querySelector('.modal-body').dataset.cardIndex = cardIndex;
    ...
    function createCardModalContent(card) {
        const jobIllustrationPath = `${imagePaths.jobIllust}/${card.role}.webp`;
        const abilitiesImages = card.abilities.map((ability, idx) => {
            let tierFolder;
            let skillChangerPath = '';
            if (idx + 1 === 1) {
                tierFolder = 'tier1';
            } else if (idx + 1 === 2) {
                tierFolder = 'tier2';
                ...
                tierFolder = 'tier3';
                skillChangerPath = `static/images/SkillChanger/3TierSkillChanger.webp`;
            } else if (idx + 1 === 4) {
                tierFolder = 'tier456';
                skillChangerPath = `static/images/SkillChanger/4TierSkillChanger.webp`;
            } else if (idx + 1 === 5) {
                tierFolder = 'tier456';
                skillChangerPath = `static/images/SkillChanger/5TierSkillChanger.webp`;
            } else {
                tierFolder = 'tier456';
                skillChangerPath = `static/images/SkillChanger/6TierSkillChanger.webp`;
            }
            const abilityImagePath = `${imagePaths.tierImage}/${tierFolder}/${ability}.webp`;
            return `<div class="ability-item">
                        <img src="${abilityImagePath}" alt="${ability}">
                        <p>${ability}</p>
                        ${skillChangerPath ? `<img src="${skillChangerPath}" alt="스킬 변경" class="skill-changer" data-tier="${idx + 1}" data-role="${card.role}" data-index="${idx}">` : ''}
                    </div>`;
        }).join('');
        ...
        return `
            <div class="job-info">
                <img src="${jobIllustrationPath}" alt="${card.role}">
                <h5>${card.role}</h5>
            </div>
    ...
    document.addEventListener('click', function(event) {
        if (event.target.classList.contains('skill-changer')) {
            const tier = parseInt(event.target.dataset.tier, 10);
            const role = event.target.dataset.role;
            const index = parseInt(event.target.dataset.index, 10);
            const cardIndex = parseInt(document.querySelector('.modal-body').dataset.cardIndex, 10);
            const card = cards[cardIndex];

            if (tier === 3) {
                changeTier3Ability(card, index);
            } else {
                changeTier456Ability(card, index, tier);
            }

            const modalContent = document.getElementById('card-modal-content');
            modalContent.innerHTML = createCardModalContent(card);
            updateCardDisplay(cards, cardContainer); // 카드 목록 갱신
        }
    });

    function changeTier3Ability(card, index) {
        const availableAbilities = card.role === "마피아" || card.role === "짐승인간"
            ? abilities.tier3_mafia
            : abilities.tier3;

        let newAbility;
        do {
            newAbility = getRandomItem(availableAbilities);
        } while (card.abilities.includes(newAbility));

        card.abilities[index] = newAbility;
    }

    function changeTier456Ability(card, index, tier) {
        let newAbility;
        do {
            newAbility = getRandomItem(tier4_6Abilities[card.role]);
        } while (card.abilities.includes(newAbility));
...
function sortCards(cards) {
    const jobOrder = ["마피아", "스파이", "짐승인간", "마담", "도둑", "마녀", "과학자", "사기꾼", "청부업자", "경찰", "자경단원", "요원", "의사", "군인", "정치인", "영매", "연인", "기자", "건달", "사립탐정", "도굴꾼", "테러리스트", "성직자", "예언자", "간호사", "판사", "마술사", "해커", "심리학자", "용병", "공무원", "비밀결사", "파파라치", "최면술사", "교주"];
...
function getRandomJob() {
    ...
    const jobs = {
        ...
        police: ["경찰", "자경단원", "요원"],
        doctor: ["의사"],
        special: ["군인", "정치인", "영매", "연인", "기자", "건달", "사립탐정", "도굴꾼", "테러리스트", "성직자", "예언자", "간호사", "판사", "마술사", "해커", "심리학자", "용병", "공무원", "비밀결사", "파파라치", "최면술사"],
        cult: ["교주"]
    };
