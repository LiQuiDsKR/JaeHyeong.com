$(document).ready(function() {
        ...
        const suggestionHtml = `
          <div class="suggestion-item ${suggestion.pinned ? 'pinned' : ''}" data-suggestion='${JSON.stringify(suggestion)}'>
            <div class="suggestion-content">
              ...
              <small>${suggestion.timestamp} | IP: 비공개 (공동 관리자가 존재함) </small>
              ${suggestion.replied ? `<div class="admin-reply"><strong>관리자 답변:</strong> <p>${suggestion.replyContent}</p></div>` : ''}
  ...
  function checkBanStatus(ip) {
    $.ajax({
      ...
      success: function(response) {
        if (response.banned) {
          // 해당 IP를 가진 suggestion-item을 찾는다
          $('.suggestion-item').each(function() {
            const suggestionData = $(this).data('suggestion');
            
            // suggestionData가 있고 IP가 같으면
            if (suggestionData && suggestionData.ip_address === ip) {
              // 이미 ban-message가 추가되어 있으면 추가하지 않음
              if ($(this).find('.ban-message').length === 0) {
                const banMessage = `
                  <div class="ban-message" style="color: red; font-weight: bold; margin-top: 10px;">
                    IP 밴: ${response.reason}
                  </div>
                `;
                $(this).find('small').after(banMessage);  // 작은 글씨 뒤에 붙이기
              }
            }
          });
        }
  ...
  $(document).on('click', '.block-btn', function() {
    ...
    if (!suggestionItem || !suggestionItem.ip_address) {
      alert('IP 정보를 찾을 수 없습니다.');
      return;
    }

    currentIpToBlock = suggestionItem.ip_address;  // data-suggestion에 있는 진짜 IP를 사용
