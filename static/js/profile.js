document.addEventListener('DOMContentLoaded', function() {
    ...
    // Unified color picker for wallet text colors
    const walletTextColorPicker = document.getElementById('wallet-text-color');
    walletTextColorPicker.addEventListener('input', updatePreview);
    ...
    function updatePreview() {
        ...
        levelElement.style.color = walletTextColorPicker.value;
        fameElement.textContent = `명성 : ${document.getElementById('fameText').value}`;
        fameElement.style.color = walletTextColorPicker.value;
        ...
        winLossElement.style.color = walletTextColorPicker.value;
        ...
        if (unrankedCheck.checked) {
            ...
            rpElement.style.color = walletTextColorPicker.value;
        ...
        bioElement.style.color = walletTextColorPicker.value;
    }

    // Color pickers
    const levelColorPicker = document.getElementById('level-color');
    const fameColorPicker = document.getElementById('fame-color');
    const winLossColorPicker = document.getElementById('winloss-color');
    const rpColorPicker = document.getElementById('rp-color');
    const bioColorPicker = document.getElementById('bio-color');

    levelColorPicker.addEventListener('input', updatePreview);
    fameColorPicker.addEventListener('input', updatePreview);
    winLossColorPicker.addEventListener('input', updatePreview);
    rpColorPicker.addEventListener('input', updatePreview);
    bioColorPicker.addEventListener('input', updatePreview);
    ...
    toggleTextColorButton.addEventListener('click', toggleTextColor);

    function toggleTextColor() {
        const textElements = document.querySelectorAll('.preview-text');
        textElements.forEach(element => {
            element.style.color = (element.style.color === 'rgb(255, 255, 255)') ? '#000000' : '#ffffff';
        });
    }
});
